...
func MigrateDatabase() {
	...
	if err != nil {
		utils.ALogger.ErrorF("failed to auto-migrate MigrationRecord table: %v", err)
		return
	}
...
func performMigration(db *gorm.DB, migrationName string, executedBy string, migrationFunc func(*gorm.DB) error) {
	...
	if result.RowsAffected == 0 {
		...
		if err != nil {
			utils.ALogger.ErrorF("migration failed (%s): %v\n", migrationName, err)
			return
		}

		// Record successful migration
		db.Create(&Changeset{Name: migrationName, RanAt: time.Now(), ExecutedBy: executedBy})
		utils.ALogger.Info("Migration executed and recorded: %s\n", migrationName)
	} else {
		utils.ALogger.Info("Migration already executed: %s\n", migrationName)
	}
}
...
func executeSQLCommands(db *gorm.DB, sqlCommands []string) error {
	for _, sql := range sqlCommands {
		if err := db.Exec(sql).Error; err != nil {
			utils.ALogger.ErrorF("failed to execute SQL command: %v\n", err)
			return err
		}
	}
	return nil
...
func renameLastSeenTableAndColumnOrCreateTable(db *gorm.DB) error {
	...
	if db.Migrator().HasTable(oldName) {
		if err := db.Migrator().RenameTable("agent_last_seens", newName); err != nil {
			utils.ALogger.ErrorF("failed to rename table: %v\n", err)
			return err
		}
		if err := db.Migrator().RenameColumn(&models.LastSeen{}, "agent_key", "key"); err != nil {
			utils.ALogger.ErrorF("failed to rename column: %v\n", err)
			return err
		}
		sqlCommands := []string{
		...
		if err == nil {
			utils.ALogger.Info("Renamed table and column successfully.")
		}
		return err
