...
func (r *AgentGroupRepository) GetByFilter(p utils.Pagination, f []utils.Filter) ([]models.AgentGroup, int64, error) {
	var groups []models.AgentGroup
	var count int64
	db := r.db
	tx := db.Model(models.AgentGroup{}).Scopes(utils.FilterScope(f)).Count(&count).Scopes(p.PagingScope).Find(&groups)
	if tx.Error != nil {
