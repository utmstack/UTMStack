        ...
        <header className="flex shrink-0 items-center gap-3 border-b border-border px-5 py-4">
          {step === 'pick' && (
            <button
              ...
              aria-label={t('common.startFrom.back')}
          ...
          <h2 className="min-w-0 flex-1 truncate text-base font-semibold">
            {step === 'choose' ? title : t('common.startFrom.pickTitle')}
          </h2>
          <button
            onClick={onClose}
        ...
        {step === 'choose' ? (
          <div className="grid gap-3 p-5 sm:grid-cols-2">
            <ChoiceCard
              ...
              title={t('common.startFrom.scratch')}
              body={t('common.startFrom.scratchHint')}
              onClick={onScratch}
            />
            <ChoiceCard
              icon={<Copy size={19} />}
              title={t('common.startFrom.copy')}
              body={t('common.startFrom.copyHint')}
              badge={t('common.startFrom.count', { n: options.length })}
          ...
          <>
            <div className="shrink-0 px-5 pt-4">
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder={t('common.startFrom.search')}
                  className="pl-8"
                  autoFocus
                />
              </div>
            ...
            <div className="mt-2 min-h-0 flex-1 overflow-y-auto px-3 pb-4">
              {shown.length === 0 ? (
                <div className="py-10 text-center text-[12px] text-muted-foreground">{t('common.startFrom.none')}</div>
              ) : (
                shown.map((o) => (
                  <button
                    key={o.id}
