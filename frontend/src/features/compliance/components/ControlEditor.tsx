          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {creating && (
              <PlatformBroadcastButton
                ...
                title={t('platformBroadcast.action.create', { resource: t('platformBroadcast.resource.control') })}
                intro={form.name || 'New Control'}
                ...
                onBroadcast={async (selector: BulkSelector) => {
                  ...
                  const payload = formToControl(f)
                  return broadcast(BULK_PATHS.compliance.controlCreate, selector, payload)
                }}
              />
            )}
            {!creating && !readOnly && (
              <>
                <PlatformBroadcastButton
                  label={t('platformBroadcast.button')}
                  title={t('platformBroadcast.action.update', { resource: t('platformBroadcast.resource.control') })}
                  intro={control?.name}
                  disabled={busy}
                  onBroadcast={async (selector: BulkSelector) => {
                    let f = form
                    ...
                    const payload = formToControl(f)
                    return broadcast(BULK_PATHS.compliance.controlUpdate, selector, payload)
                  }}
                />
                <PlatformBroadcastButton
                  label={t('platformBroadcast.button')}
                  title={t('platformBroadcast.action.delete', { resource: t('platformBroadcast.resource.control') })}
                  intro={control?.name}
                  disabled={busy}
                  onBroadcast={async (selector: BulkSelector) => {
                    return broadcast(BULK_PATHS.compliance.controlDelete, selector, { id: control!.id })
                  }}
                />
              </>
            )}
