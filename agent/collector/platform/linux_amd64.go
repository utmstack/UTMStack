...
const (
	journaldRestartDelay    = 5 * time.Second
	journaldMaxRestartDelay = 5 * time.Minute
)
...
func (l *LinuxSystem) Start(ctx context.Context, enqueue func(*plugins.Log) error) {
	...
	restartDelay := journaldRestartDelay
	...
	for {
		...
		// Exponential backoff
		restartDelay *= 2
		if restartDelay > journaldMaxRestartDelay {
			restartDelay = journaldMaxRestartDelay
		}
	}
}

func (l *LinuxSystem) runJournalctl(ctx context.Context, host string, enqueue func(*plugins.Log) error) int {
	l.mu.Lock()
