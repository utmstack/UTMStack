...
func (r *pgIncidentNoteRepository) FindAll(ctx context.Context, q dto.IncidentNoteListQuery) ([]domain.UtmIncidentNote, int64, error) {
	if q.Page < 1 {
		q.Page = 1
	}
	...
	var rows []domain.UtmIncidentNote
	if err := db.Order("note_send_date DESC").
