...
const bitdefenderEndpointPush = "/v1.0/jsonrpc/push"
...
// verifyBitdefender — ported from modulekinds/bitdefender/validate.go.
func verifyBitdefender(c map[string]string) error {
	if err := required(c, "Bitdefender",
		"connectionKey",
		"accessUrl",
		"utmPublicIP",
		"companyIds",
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, accessUrl+bitdefenderEndpointPush, bytes.NewBuffer(body))
	if err != nil {
