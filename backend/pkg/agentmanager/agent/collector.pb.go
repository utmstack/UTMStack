...
// IntegrationStateRequest asks for the persisted desired-state row of one
// (collector, data_type) pair. Pure read of collector_integration_config —
// never touches the live CollectorStream.
...
// IntegrationStateResponse mirrors the persisted collector_integration_config
// row. configured=false means no row exists yet for this (collector_id,
// data_type) pair (all other fields are then zero-valued). configurations
// reuses CollectorGroupConfigurations' conf_key/conf_value vocabulary
// (enabled/proto/port/tls/auth/path/signature_header), same as the desired
// state pushed by SetCollectorConfig.
...
const file_collector_proto_rawDesc = "" +
	...
	"\x1cGetCollectorIntegrationState\x12\x1e.agent.IntegrationStateRequest\x1a\x1f.agent.IntegrationStateResponse\"\x00B2Z0github.com/utmstack/UTMStack/agent-manager/agentb\x06proto3"
