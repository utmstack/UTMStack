...
public class PdfGeneratorResource {
    ...
    public ResponseEntity getPdfReportInBytes(@RequestParam String url,
        ...
        try {
            ...
            String urlParam = String.format("%s?token=%s&url=%s", Constants.FRONT_BASE_URL, accessKey.substring(7), url);
            byte [] resultPdf = pdfService.getPdf(urlParam, accessKey.substring(7), accessType.get());
