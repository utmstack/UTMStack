...
const manualConfigTemplate = `# AS400 servers to collect from. The collector watches this file and reloads
# automatically on change — no restart needed. All fields are required per server.
servers: []
#  - tenant: default
#    hostname: 10.0.0.5
#    userId: QSECOFR
...
// CreateManualConfigTemplate writes the editable servers config template on
// install (only if absent, so it is never overwritten).
func CreateManualConfigTemplate() error {
	if utils.CheckIfPathExist(ManualConfigFile) {
		return nil
	}
	return os.WriteFile(ManualConfigFile, []byte(manualConfigTemplate), 0644)
}
