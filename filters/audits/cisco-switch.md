...
nothing else. The schema is ThreatWinds go-sdk **v1.1.33**, as pinned by `plugins/alerts/go.mod`.
...
- **What the corrections rest on:** the real text shapes above; the public EventProcessor at
  ...
  disables a rule at its fifth error with a `Circuit Breaker: <rule name>` alert; and go-sdk
  v1.1.33, whose [`plugins/cel.go`](https://github.com/threatwinds/go-sdk/blob/v1.1.33/plugins/cel.go)
...
- The playground's parser and writer plugins link go-sdk v1.1.26 and its CEL plugin v1.1.34; the
...
Build the EventProcessor commit above without changing its dependencies. With `EP` set to that
