# Generic Input normalization and rule review

Use the SDK fieldName key so message extraction is retained.

This draft targets UTMStack `v11`. It contains 1 filter changes
and 0 rule changes for this technology only. Review covered
1 filter configurations and 1 matching shipped rule files.
...
`plugins/alerts/testdata/filter-contracts/generic.json`. Apply that support before running `go test ./...` in `plugins/alerts`.
...
The one-field message capture is validated as a copy and the configuration decodes strictly against the SDK.
