# FortiGate normalization and rule review

Classify quoted policy decisions after cleanup; security-profile denial wins; connection closure is separate from successful traffic.
...
`plugins/alerts/testdata/filter-contracts/fortigate.json`. Apply that support before running `go test ./...` in `plugins/alerts`.
...
A policy accept result means the firewall permitted the action; it does not prove TCP handshake completion. Explicit UTM denial wins over accept, while reset/close/timeout describe closure without fabricated success.
