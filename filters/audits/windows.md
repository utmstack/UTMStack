# Windows normalization and rule review

Correct successful account administration and NTLM status; preserve actor roles/IPs and align alert grouping.

This draft targets UTMStack `v11`. It contains 1 filter changes
and 15 rule changes for this technology only. Review covered
1 filter configurations and 38 matching shipped rule files.
...
`plugins/alerts/testdata/filter-contracts/windows.json`. Apply that support before running `go test ./...` in `plugins/alerts`.
