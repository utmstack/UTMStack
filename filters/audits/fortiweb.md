# FortiWeb normalization and rule review

Normalize denial/IP values and repair the web-shell rule CEL regex escapes.
...
`plugins/alerts/testdata/filter-contracts/fortiweb.json`. Apply that support before running `go test ./...` in `plugins/alerts`.
