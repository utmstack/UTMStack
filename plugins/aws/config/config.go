...
func StartConfigurationSystem() {
	...
	for {
		...
		if err != nil {
			catcher.Error("Failed to connect to server", err, map[string]any{"process": "plugin_com.utmstack.aws"})
			cancel()
			time.Sleep(reconnectDelay)
			continue
		}
		...
		if state == connectivity.Shutdown || state == connectivity.TransientFailure {
			catcher.Error("Connection is in shutdown or transient failure state", nil, map[string]any{"process": "plugin_com.utmstack.aws"})
			cancel()
			time.Sleep(reconnectDelay)
			continue
		}
		...
		if err != nil {
			catcher.Error("Failed to create stream", err, map[string]any{"process": "plugin_com.utmstack.aws"})
			conn.Close()
			cancel()
			time.Sleep(reconnectDelay)
			continue
		...
		err = stream.Send(&BiDirectionalMessage{
			Payload: &BiDirectionalMessage_PluginInit{
				PluginInit: &PluginInit{Type: PluginType_AWS_IAM_USER},
			},
		})
		if err != nil {
			catcher.Error("Failed to send PluginInit", err, map[string]any{"process": "plugin_com.utmstack.aws"})
			conn.Close()
			cancel()
			time.Sleep(reconnectDelay)
			continue
		...
		for {
			...
			if err != nil {
				if strings.Contains(err.Error(), "EOF") {
					catcher.Info("Stream closed by server, reconnecting...", map[string]any{"process": "plugin_com.utmstack.aws"})
					conn.Close()
					cancel()
					time.Sleep(reconnectDelay)
					break
				...
				if ok && (st.Code() == codes.Unavailable || st.Code() == codes.Canceled) {
					catcher.Error("Stream error: "+st.Message(), err, map[string]any{"process": "plugin_com.utmstack.aws"})
					conn.Close()
					cancel()
					time.Sleep(reconnectDelay)
					break
				} else {
					catcher.Error("Stream receive error", err, map[string]any{"process": "plugin_com.utmstack.aws"})
					time.Sleep(reconnectDelay)
					continue
				}
			}
