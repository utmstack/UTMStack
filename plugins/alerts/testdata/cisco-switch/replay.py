"""Replay fabricated Cisco switch raw lines through separately built EventProcessor binaries.

Requires PyYAML. This does not build or deploy anything and uses only local file writers.
It stages the current filter, the shared grok definitions (patterns.yaml) and the three Cisco
switch rules, runs the playground once, and checks every event against expected.json and every
local alert against its expected rule. The MAC spoofing and ARP poisoning rules have history
searches. Their OpenSearch address is a closed local port: no fixture may reach a history
search, and an attempt would fail and be reported. Every input is fabricated. See
...
PLUGINS = ("add", "cast", "cel", "delete", "grok", "saw", "sew", "trim")
TENANT = "00000000-0000-4000-8000-000000000001"
ENVELOPE_TIMESTAMP = "2026-09-24T14:00:00Z"
ENVELOPE = ("id", "timestamp", "deviceTime", "dataType", "dataSource", "tenantId", "tenantName", "raw", "errors")
LOG_FAILURES = ("failed to unmarshal", "failed to evaluate", "failed to execute correlation search",
...
def run(playground, plugins):
    ...
    port = closed_port()
    require(not listening(port), f"Port {port} is in use")
    config = {
    ...
    filter_path = root / "filters/cisco/cs_switch.yml"
    shutil.copy2(filter_path, work / "pipeline/filters/cs_switch.yaml")
    rule_paths = sorted((root / "rules/cisco/cs_switch").glob("*.y*ml"))
    require(len(rule_paths) == 3, f"Expected 3 Cisco switch rules, found {len(rule_paths)}")
    stems = {}
    ...
    for number, name in enumerate(sorted(cases)):
        event_id = f"cisco-switch-{name}"
        inputs[event_id] = (name, cases[name])
        event = {"id": event_id, "dataType": "cisco-switch", "dataSource": "fixture-switch",
                 "@timestamp": ENVELOPE_TIMESTAMP, "tenantId": TENANT, "raw": cases[name]}
        (work / "input" / f"{number:03d}.json").write_text(json.dumps(event))
    hashed = [filter_path, *rule_paths] + [fixture_dir / n for n in ("patterns.yaml", "raw.json")]
    manifest = {
    ...
    require(not listening(port), f"Something started listening on port {port} during the run")
...
def main():
    ...
    work, inputs, stems, events, alerts, log_text = run(args.playground, args.plugins)
