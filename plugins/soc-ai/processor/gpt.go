...
import (
	...
	"github.com/utmstack/UTMStack/plugins/soc-ai/gpt"
...
func (p *Processor) processGPTRequests() {
	for alert := range p.GPTQueue {
		...
		if err != nil {
			p.RegisterError(fmt.Sprintf("error while making request to GPT: %v", err), alert.ID)
			continue
		}

		response, err = clearJson(response)
		if err != nil {
			p.RegisterError(fmt.Sprintf("error while cleaning json response: %v", err), alert.ID)
			continue
		}

		alertResponse, err := utils.ConvertFromJsonToStruct[schema.GPTAlertResponse](response)
		if err != nil {
			p.RegisterError(fmt.Sprintf("error while converting response to struct: %v", err), alert.ID)
			continue
		}
